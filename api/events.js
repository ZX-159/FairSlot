...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          cover_url: cover_url ? String(cover_url).slice(0, 500) : '',
    ...
    if (req.method === 'PUT') {
      const body = req.body || {};
      const { id, title, description, location, event_date, cover_url, category, status, locked, settings } =
        body;
      ...
      if (!existing.locked) {
        ...
        if (location !== undefined) patch.location = String(location).slice(0, 300);
        if (event_date !== undefined) patch.event_date = event_date || null;
        if (cover_url !== undefined) patch.cover_url = String(cover_url).slice(0, 500);
    ...
    if (req.method === 'DELETE') {
      ...
      // FK cascades handle slots/claims/settings if schema has ON DELETE CASCADE;
      // explicit deletes keep older DBs consistent.
